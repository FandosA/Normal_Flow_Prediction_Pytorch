...
dataset_output_file = "dataset_autoencoder.json"
scenearios = os.listdir(dataset_path)
...
for i, sceneario in enumerate(scenearios):
    ...
    images_path = os.path.join(dataset_path, sceneario, "image_left")
    images_names = os.listdir(images_path)
    optical_flow_path = os.path.join(dataset_path, sceneario, "flow")
    ...
    for i in range(len(images_names) - 1):
        ...
        data_autoencoder["flow"].append(os.path.join(optical_flow_path, flow_files[2 * i]))

# Save data in json file
with open(dataset_output_file, "w") as fp:
